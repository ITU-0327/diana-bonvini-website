...
<div style="font-family: Arial, Helvetica, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
    <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="color: #2563eb; margin-bottom: 10px;">Time Slots Available</h1>
        <p style="font-size: 16px; color: #666;">Writing Service Request #<?= h($writing_service_request->writing_service_request_id) ?></p>
    </div>
    ...
    <div style="background-color: #f3f4f6; border-radius: 8px; padding: 20px; margin-bottom: 25px;">
        ...
        <p><?= h($admin_name) ?> has sent you available time slots for your writing consultation.</p>
        
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            <tr>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-weight: bold; width: 150px;">Service:</td>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><?= h($writing_service_request->service_title) ?></td>
            </tr>
            <tr>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Request ID:</td>
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><?= h($writing_service_request->writing_service_request_id) ?></td>
            </tr>
            <tr>
    ...
    <div style="background-color: #eff6ff; border-left: 4px solid #2563eb; padding: 15px; margin-bottom: 25px;">
        <h3 style="color: #1d4ed8; margin-top: 0; font-size: 16px;">Select Your Preferred Time</h3>
        ...
        <p style="margin-top: 10px;">
            <a href="<?= $this->Url->build(['controller' => 'WritingServiceRequests', 'action' => 'view', $writing_service_request->writing_service_request_id, 'prefix' => false], ['fullBase' => true]) ?>" style="display: inline-block; background-color: #2563eb; color: #fff; padding: 10px 15px; text-decoration: none; border-radius: 5px; font-weight: bold;">View & Select Time</a>
        </p>
    </div>
    
    <div style="font-size: 12px; color: #666; text-align: center; padding-top: 20px; border-top: 1px solid #e5e7eb;">
        <p>This is an automated message. Please do not reply directly to this email.</p>
        <p>&copy; <?= date('Y') ?> Diana Bonvini Writing Services. All rights reserved.</p>
    </div>
</div> 